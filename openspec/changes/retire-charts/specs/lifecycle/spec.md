...
### Requirement: @vanduo-oss/charts SHALL be retired with no further releases

As of 2026-07-25 the Vanduo legacy (dual-engine) line is retired. `0.2.0` is the final
published version of `@vanduo-oss/charts`. The repository SHALL be
archived read-only, and no subsequent version SHALL be published.
The successor is `@vanduo-oss/vd3-cbun/charts`.

#### Scenario: New work is proposed against this repository
- **WHEN** a feature, fix, or OpenSpec change is proposed for @vanduo-oss/charts
- **THEN** it SHALL be redirected to the perspective line (`vd3` / `vd3-cbun` / `vd3-docs`)
- **AND** no new version of @vanduo-oss/charts SHALL be published
...
- **WHEN** a consumer runs `pnpm add @vanduo-oss/charts`
- **THEN** the install SHALL succeed and resolve `0.2.0`
...
- **AND** it SHALL name the final version and the `retired-v0.2.0` git tag
